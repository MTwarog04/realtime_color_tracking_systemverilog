...
module mask_despeckle_filter #(
    ...
    always_comb begin
        ...
        if (pix_x >= 1) begin
            ...
            if (pix_y >= 1) begin
                neighbour_count = neighbour_count + line_1_left_1;
            end
            if (pix_y >= 2) begin
                neighbour_count = neighbour_count + line_2_left_1;
            end
        end
        if (pix_x >= 2) begin
            neighbour_count = neighbour_count + current_left_2;
            if (pix_y >= 1) begin
                neighbour_count = neighbour_count + line_1_left_2;
            end
            if (pix_y >= 2) begin
                neighbour_count = neighbour_count + line_2_left_2;
            end
        end
        if (pix_y >= 1) begin
            neighbour_count = neighbour_count + line_1_current;
        end
        if (pix_y >= 2) begin
            neighbour_count = neighbour_count + line_2_current;
        end
